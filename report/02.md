...
在上述特征重要性排名中，`text_svd_1`（第 6 位）和 `text_svd_2`（第 10 位）是由文本字段 `material_source_and_purity`（原料来源与纯度描述）经 TF-IDF 向量化后再通过截断 SVD（TruncatedSVD, 16 维）降维所得的潜在语义成分。为理解其物理含义，我们通过 `inspect_text_svd.py` 脚本反向解析了 SVD 分量对应的高权重词汇（完整结果见 `results/text_svd_interpretation.txt`），关键发现如下：
...
为进一步理解文本特征在模型中的作用，我们利用 `inspect_text_svd.py` 脚本对 TruncatedSVD 降维后的全部 **16 个潜在语义分量**（`text_svd_0` ~ `text_svd_15`）进行了逆向词权重分析（完整输出见 `results/text_svd_interpretation.txt`）。以下将各分量按其捕捉的主要语义维度归纳为四大主题簇。
